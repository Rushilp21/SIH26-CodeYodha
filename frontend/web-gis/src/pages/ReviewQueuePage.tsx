  ...
  return (
    <div className="review-workspace">
      <section className="review-main">
        <div className="review-header"><div><Link className="back-link" to={`/projects/${projectId}`}>← Map workspace</Link><h1>Review prioritization queue</h1><p>Confidence, health, topology evidence, RL refinement, anomalies, and survey priority come from the target APIs.</p></div><button type="button" className="export-labels-button" disabled={exporting || reviewedTotal === 0} onClick={downloadLabels}>{exporting ? "Preparing export…" : "Export reviewed labels"}</button></div>
        ...
        {workspace.data && !workspace.loading && !workspace.error ? <>
          ...
          <p className="panel-note">All scores and refinement states below are stored backend evidence. Human false-positive labels are written back as review evidence and anomaly records.</p>
...
function ReviewDetail({ projectId, selected, assignee, onAssignee, assigning, message, onAssign, onClose }: { projectId: string; selected: ReviewRowData | null; assignee: string; onAssignee: (value: string) => void; assigning: boolean; message: string | null; onAssign: () => void; onClose: () => void }) {
  ...
  return <aside className="review-panel"><div className="parcel-panel-header"><div><strong>{selected.parcel.id.slice(0, 12)}</strong><span>{selected.parcel.status.replace("_", " ")} · {selected.parcel.source.replace("_", " ")}</span></div><button type="button" className="panel-close" onClick={onClose} aria-label="Close review details">×</button></div><div className="review-panel-body"><section><h2>Review evidence</h2><Info label="Confidence" value={formatPercent(selected.parcel.confidence_score)} /><Info label="Cadastral health" value={formatPercent(selected.parcel.health_score)} /><Info label="Segmentation" value={formatEvidence(segmentation)} /><Info label="Topology" value={formatEvidence(topology)} /><Info label="Edge alignment" value={formatEvidence(alignment)} /><Info label="Baseline agreement" value={formatEvidence(baseline)} /><Info label="PPO refinement" value={formatEvidence(rl)} /></section><section><h2>Detected anomalies & labels</h2>{selected.anomalies.length ? selected.anomalies.map((anomaly, index) => <div className="anomaly-card" key={`${anomaly.type}-${index}`}><strong>{anomaly.type}</strong><span>Magnitude {anomaly.magnitude.toFixed(2)}</span></div>) : <p className="panel-note">No anomalies or human false-positive labels returned for this parcel.</p>}</section><section><h2>Actions</h2><div className="review-actions"><Link className="action-primary" to={`/projects/${projectId}/parcels/${selected.parcel.id}`}>Open parcel & verify</Link><Link className="action-secondary" to={`/projects/${projectId}?parcel=${encodeURIComponent(selected.parcel.id)}`}>Locate on map</Link></div>{selected.queueItem ? <div className="assignment-box"><label htmlFor="assignee">Assign survey queue item</label><input id="assignee" value={assignee} onChange={(event) => onAssignee(event.target.value)} placeholder="Assignee UUID" /><button type="button" disabled={!assignee.trim() || assigning} onClick={onAssign}>{assigning ? "Assigning…" : "Save assignment"}</button><p>Uses the existing survey-queue assignment API.</p></div> : <p className="panel-note">This parcel has no survey-queue item to assign.</p>}{message ? <p className="action-feedback">{message}</p> : null}</section></div></aside>;
}
...
function formatEvidence(component: Explanation["components"][number] | undefined) { return component ? `${Math.round(component.score * 100)}% · ${component.explanation}` : "No stored evidence"; }
