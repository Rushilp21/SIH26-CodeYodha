...
export function ParcelDetailsPanel({ projectId, parcel, explanation, explanationLoading, explanationError, anomaliesLoading, anomaliesError, anomalies, verifying, feedback, onClose, onVerify, onReject }: Props) {
  ...
  const stage = refinement ? refinement.explanation : `Recorded source: ${parcel.source.split("_").join(" ")}. No stored PPO evidence was returned.`;
  ...
  return <aside className="parcel-panel">
    ...
    <div className="parcel-panel-body">
      ...
      <PanelSection title="AI processing stage"><div className="pipeline-stages"><span className="done">Imported</span><span className={isAi ? "done" : ""}>AI extracted</span><span className={refinement ? "done current" : ""}>{refinement ? "PPO refined" : "RL unavailable"}</span><span className={topology ? "done" : ""}>{topology ? "Topology checked" : "Validation unavailable"}</span><span className={parcel.status === "verified" ? "done current" : ""}>Verified</span></div><p className="panel-note">{stage}</p></PanelSection>
      ...
      <PanelSection title="Topology & refinement">{explanationError && <p role="alert" className="panel-note">Evidence unavailable: {explanationError}</p>}{explanationLoading ? <p className="panel-note">Loading refinement evidence…</p> : null}<Info label="Segmentation alignment" value={segmentation ? `${percentage(segmentation.score)}%` : "Not returned"} /><Info label="Topology validity" value={topology ? `${percentage(topology.score)}%` : "Not returned"} /><Info label="Edge alignment" value={edgeAlignment ? `${percentage(edgeAlignment.score)}%` : "Not returned"} /><Info label="Baseline agreement" value={baselineAgreement ? `${percentage(baselineAgreement.score)}%` : "Not returned"} /><Info label="PPO refinement" value={refinement ? `${percentage(refinement.score)}% · stored execution evidence` : "Not returned"} /></PanelSection>
